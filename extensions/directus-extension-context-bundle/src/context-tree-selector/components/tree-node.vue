...
<template>
  <ul
    ...
    <li class="tree-node">
      <button
        @click="store.toggleNodeCollapse(node.id)"
        :class="!node.children || node.children.length === 0 ? 'visibility-hidden' : ''">
      ...
      <tree-node
        v-if="!store.getIsCollapsed(node.id) && node.children && node.children.length"
        :nodes="node.children" />
    </li>
  </ul>
</template>
